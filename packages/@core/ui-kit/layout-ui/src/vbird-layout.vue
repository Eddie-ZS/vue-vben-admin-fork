...
import type { VbirdLayoutProps } from './vbird-layout';

import { computed, ref, watch } from 'vue';

import { SCROLL_FIXED_CLASS, useLayoutFooterStyle, useLayoutHeaderStyle } from '@vbird-core/composables';
import { Menu } from '@vbird-core/icons';
import { VbirdIconButton } from '@vbird-core/shadcn-ui';
import { ELEMENT_ID_MAIN_CONTENT } from '@vbird-core/shared/constants';
...
interface Props extends VbirdLayoutProps {}
	...
	if (
		...
		if (isSideNavEffective) {
			...
			sidebarAndExtraWidth =
				sidebarExpandOnHovering.value && !sidebarExpandOnHover.value ? `${getSideCollapseWidth.value}px` : `${getSidebarWidth.value}px`;
			width = `calc(100% - ${sidebarAndExtraWidth})`;
		}
	}
	return {
...
const showHeaderToggleButton = computed(() => {
	return (
		props.isMobile ||
		(props.headerToggleSidebarButton && isSideMode.value && !isSidebarMixedNav.value && !isMixedNav.value && !props.isMobile)
	);
});
			...
			<div
				...
				<LayoutHeader
					...
					<template #toggle-button>
						<VbirdIconButton v-if="showHeaderToggleButton" class="my-0 mr-1 rounded-md" @click="handleHeaderToggle">
							<Menu class="size-4" />
						</VbirdIconButton>
					</template>
