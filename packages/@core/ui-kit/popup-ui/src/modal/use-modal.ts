...
import VbirdModal from './modal.vue';

const USER_MODAL_INJECT_KEY = Symbol('VBIRD_MODAL_INJECT');

export function useVbirdModal<TParentModalProps extends ModalProps = ModalProps>(options: ModalApiOptions = {}) {
...
async function checkProps(api: ExtendedModalApi, attrs: Record<string, any>) {
	...
	for (const attr of Object.keys(attrs)) {
		if (stateKeys.has(attr) && !['class'].includes(attr)) {
			...
			console.warn(
				`[Vbird Modal]: When 'connectedComponent' exists, do not set props or slots '${attr}', which will increase complexity. If you need to modify the props of Modal, please use useVbirdModal or api.`
			);
		}
	}
}
