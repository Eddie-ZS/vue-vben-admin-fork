...
import { useIsMobile, usePriorityValues, useSimpleLocale } from '@vbird-core/composables';
import { Expand, Shrink } from '@vbird-core/icons';
import {
	Dialog,
	DialogContent,
	DialogDescription,
	...
	<Dialog :modal="false" :open="state?.isOpen" @update:open="() => modalApi?.close()">
		<DialogContent
				...
				<DialogTitle v-if="title" class="text-left">
					<slot name="title">
						...
						<slot v-if="titleTooltip" name="titleTooltip">
							<VbirdHelpTooltip trigger-class="pb-1">
								{{ titleTooltip }}
							</VbirdHelpTooltip>
						</slot>
					</slot>
			...
			<div
				...
				">
				<VbirdLoading v-if="showLoading" class="size-full h-auto min-h-full" spinning />
				<slot></slot>
			</div>
			...
			<DialogFooter
				...
				<slot name="footer">
					<component
						:is="components.DefaultButton || VbirdButton"
					...
					<component
						:is="components.PrimaryButton || VbirdButton"
