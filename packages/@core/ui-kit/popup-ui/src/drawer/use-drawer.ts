...
import VbirdDrawer from './drawer.vue';

const USER_DRAWER_INJECT_KEY = Symbol('VBIRD_DRAWER_INJECT');

export function useVbirdDrawer<TParentDrawerProps extends DrawerProps = DrawerProps>(options: DrawerApiOptions = {}) {
	...
	const Drawer = defineComponent(
		(props: DrawerProps, { attrs, slots }) => {
			return () => h(VbirdDrawer, { ...props, ...attrs, drawerApi: extendedApi }, slots);
		},
		{
			inheritAttrs: false,
			name: 'VbirdDrawer'
...
async function checkProps(api: ExtendedDrawerApi, attrs: Record<string, any>) {
	...
	for (const attr of Object.keys(attrs)) {
		if (stateKeys.has(attr) && !['class'].includes(attr)) {
			// connectedComponent存在时，不要传入Drawer的props，会造成复杂度提升，如果你需要修改Drawer的props，请使用 useVbirdDrawer 或者api
			console.warn(
				`[Vbird Drawer]: When 'connectedComponent' exists, do not set props or slots '${attr}', which will increase complexity. If you need to modify the props of Drawer, please use useVbirdDrawer or api.`
			);
		}
	}
}
