...
import { ChevronDown } from '@vbird-core/icons';
					...
					<BreadcrumbItem>
						<div v-if="item.items?.length ?? 0 > 0">
							<DropdownMenu>
								<DropdownMenuTrigger class="flex items-center gap-1">
									<VbirdIcon v-if="showIcon" :icon="item.icon" class="size-5" />
									{{ item.title }}
						...
						<BreadcrumbLink v-else-if="index !== breadcrumbs.length - 1" href="javascript:void 0" @click.stop="handleClick(item.path)">
							<div class="flex-center">
								<VbirdIcon v-if="showIcon" :class="{ 'size-5': item.isHome }" :icon="item.icon" class="mr-1 size-4" />
								{{ item.title }}
							</div>
						</BreadcrumbLink>
