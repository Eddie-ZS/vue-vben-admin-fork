...
import type { Recordable } from '@vbird-core/typings';

import type { FormActions, FormSchema, VbirdFormProps } from './types';

import { toRaw } from 'vue';

import { Store } from '@vbird-core/shared/store';
import {
	bindMethods,
	createMerge,
	isDate,
	...
	StateHandler
} from '@vbird-core/shared/utils';

function getDefaultState(): VbirdFormProps {
	return {
		actionWrapperClass: '',
		collapsed: false,
		collapsedRows: 1,
...
export class FormApi {
	// private api: Pick<VbirdFormProps, 'handleReset' | 'handleSubmit'>;
	public form = {} as FormActions;
	isMounted = false;

	public state: null | VbirdFormProps = null;
	stateHandler: StateHandler;

	public store: Store<VbirdFormProps>;

	// 最后一次点击提交时的表单值
	private latestSubmissionValues: null | Recordable<any> = null;

	private prevState: null | VbirdFormProps = null;

	constructor(options: VbirdFormProps = {}) {
		const { ...storeState } = options;

		const defaultState = getDefaultState();

		this.store = new Store<VbirdFormProps>(
			{
				...defaultState,
				...storeState
			},
	...
	setState(stateOrFn: ((prev: VbirdFormProps) => Partial<VbirdFormProps>) | Partial<VbirdFormProps>) {
	...
	private async getForm() {
		...
		if (!this.form?.meta) {
			throw new Error('<VbirdForm /> is not mounted');
		}
		return this.form;
	}
