...
import { Bell, MailCheck } from '@vbird/icons';
import { $t } from '@vbird/locales';

import { VbirdButton, VbirdIconButton, VbirdPopover, VbirdScrollbar } from '@vbird-core/shadcn-ui';
...
<template>
	<VbirdPopover v-model:open="open" content-class="relative right-2 w-[360px] p-0">
		<template #trigger>
			<div class="flex-center mr-2 h-full" @click.stop="toggle()">
				<VbirdIconButton class="bell-button text-foreground relative">
		...
		<div class="relative">
			<div class="flex items-center justify-between p-4 py-3">
				...
				<VbirdIconButton :disabled="notifications.length <= 0" :tooltip="$t('ui.widgets.markAllAsRead')" @click="handleMakeAll">
					<MailCheck class="size-4" />
				</VbirdIconButton>
			</div>
			<VbirdScrollbar v-if="notifications.length > 0">
			...
			<div class="border-border flex items-center justify-between border-t px-4 py-3">
				<VbirdButton :disabled="notifications.length <= 0" size="sm" variant="ghost" @click="handleClear">
					{{ $t('ui.widgets.clearNotifications') }}
				</VbirdButton>
				<VbirdButton size="sm" @click="handleViewAll">
