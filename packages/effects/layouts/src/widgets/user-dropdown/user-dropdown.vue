...
import type { AnyFunction } from '@vbird/types';

import { computed, useTemplateRef, watch } from 'vue';

import { useHoverToggle } from '@vbird/hooks';
import { LockKeyhole, LogOut } from '@vbird/icons';
import { $t } from '@vbird/locales';
import { preferences, usePreferences } from '@vbird/preferences';
import { useLockStore } from '@vbird/stores';
import { isWindowsOs } from '@vbird/utils';
...
const [LockModal, lockModalApi] = useVbirdModal({
	connectedComponent: LockScreenModal
});
const [LogoutModal, logoutModalApi] = useVbirdModal({
	onConfirm() {
		handleSubmitLogout();
	}
});
		...
		<DropdownMenuTrigger ref="refTrigger" :disabled="props.trigger === 'hover'">
			<div class="hover:bg-accent ml-1 mr-2 cursor-pointer rounded-full p-1.5">
				<div class="hover:text-accent-foreground flex-center">
					<VbirdAvatar :alt="text" :src="avatar" class="size-8" dot />
				</div>
			</div>
		</DropdownMenuTrigger>
		<DropdownMenuContent class="mr-2 min-w-[240px] p-0 pb-1">
			<div ref="refContent">
				<DropdownMenuLabel class="flex items-center p-3">
					<VbirdAvatar :alt="text" :src="avatar" class="size-12" dot dot-class="bottom-0 right-1 border-2 size-4 bg-green-500" />
				...
				<DropdownMenuItem
					...
					@click="menu.handler">
					<VbirdIcon :icon="menu.icon" class="mr-2 size-4" />
					{{ menu.text }}
				</DropdownMenuItem>
				<DropdownMenuSeparator />
				<DropdownMenuItem
