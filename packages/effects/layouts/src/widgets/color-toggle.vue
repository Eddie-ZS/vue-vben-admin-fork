...
import type { BuiltinThemeType } from '@vbird/types';

import { Palette } from '@vbird/icons';
import { COLOR_PRESETS, preferences, updatePreferences } from '@vbird/preferences';
	...
	<div class="group relative flex items-center overflow-hidden">
		<div class="flex w-0 overflow-hidden transition-all duration-500 ease-out group-hover:w-60">
			<template v-for="preset in COLOR_PRESETS" :key="preset.color">
				<VbirdIconButton class="flex-center flex-shrink-0" @click="handleUpdate(preset.color, preset.type)">
