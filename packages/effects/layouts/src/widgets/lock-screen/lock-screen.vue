...
import { LockKeyhole } from '@vbird/icons';
import { $t, useI18n } from '@vbird/locales';
import { storeToRefs, useLockStore } from '@vbird/stores';

import { useScrollLock } from '@vbird-core/composables';
...
const [Form, { form, validate }] = useVbirdForm(
	reactive({
		commonConfig: {
			hideLabel: true,
		...
		<transition name="slide-right">
			<div v-if="showUnlockForm" class="flex-center size-full" @keydown.enter.prevent="handleSubmit">
				<div class="flex-col-center mb-10 w-[300px]">
					<VbirdAvatar :src="avatar" class="enter-x mb-6 size-20" />

					<div class="enter-x mb-2 w-full items-center">
						<Form />
					</div>
					<VbirdButton class="enter-x w-full" @click="handleSubmit">
						{{ $t('ui.widgets.lockScreen.entry') }}
					</VbirdButton>
					<VbirdButton class="enter-x my-2 w-full" variant="ghost" @click="$emit('toLogin')">
						{{ $t('ui.widgets.lockScreen.backToLogin') }}
					</VbirdButton>
					<VbirdButton class="enter-x mr-2 w-full" variant="ghost" @click="toggleUnlockForm">
						{{ $t('common.back') }}
					</VbirdButton>
				</div>
			</div>
