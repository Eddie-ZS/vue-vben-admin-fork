...
import type { SupportedLanguagesType } from '@vbird/locales';
import type {
	BreadcrumbStyleType,
	BuiltinThemeType,
	ContentCompactType,
...
import type { SegmentedItem } from '@vbird-core/shadcn-ui';

import { computed, ref } from 'vue';

import { Copy, RotateCw } from '@vbird/icons';
import { $t, loadLocaleMessages } from '@vbird/locales';
import { clearPreferencesCache, preferences, resetPreferences, usePreferences } from '@vbird/preferences';

import { useVbirdDrawer } from '@vbird-core/popup-ui';
import { VbirdButton, VbirdIconButton, VbirdSegmented } from '@vbird-core/shadcn-ui';
...
const { diffPreference, isDark, isFullContent, isHeaderNav, isHeaderSidebarNav, isMixedNav, isSideMixedNav, isSideMode, isSideNav } =
	usePreferences();
const { copy } = useClipboard({ legacy: true });

const [Drawer] = useVbirdDrawer();
		...
		<Drawer :description="$t('preferences.subtitle')" :title="$t('preferences.title')" class="sm:max-w-sm">
			<template #extra>
				<div class="flex items-center">
					<VbirdIconButton :disabled="!diffPreference" :tooltip="$t('preferences.resetTip')" class="relative">
			...
			<div class="p-1">
				<VbirdSegmented v-model="activeTab" :tabs="tabs">
					<template #general>
			...
			<template #footer>
				<VbirdButton :disabled="!diffPreference" class="mx-4 w-full" size="sm" variant="default" @click="handleCopy">
					<Copy class="mr-2 size-3" />
					{{ $t('preferences.copyPreferences') }}
				</VbirdButton>
				<VbirdButton :disabled="!diffPreference" class="mr-4 w-full" size="sm" variant="ghost" @click="handleClearCache">
