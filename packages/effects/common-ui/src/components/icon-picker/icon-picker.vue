...
import { usePagination } from '@vbird/hooks';
import { EmptyIcon, Grip, listIcons } from '@vbird/icons';
...
<template>
	<VbirdPopover v-model:open="visible" :content-props="{ align: 'end', alignOffset: -11, sideOffset: 8 }" content-class="p-0 pt-3">
		<template #trigger>
			<component :is="inputComponent" :[modelValueProp]="currentSelect" :placeholder="$t('ui.iconPicker.placeholder')">
				<template #[iconSlot]>
					<VbirdIcon :icon="currentSelect || Grip" class="size-4" />
				</template>
			</component>
		</template>
		...
		<template v-if="paginationList.length > 0">
			<div class="grid max-h-[360px] w-full grid-cols-6 justify-items-center">
				<VbirdIconButton v-for="(item, index) in paginationList" :key="index" :tooltip="item" tooltip-side="top" @click="handleClick(item)">
					<VbirdIcon
						:class="{
