...
import { VBIRD_DOC_URL, VBIRD_GITHUB_URL, VBIRD_PREVIEW_URL } from '@vbird/constants';

import { VbirdRenderContent } from '@vbird-core/shadcn-ui';
...
declare global {
	const __VBIRD_ADMIN_METADATA__: {
		authorEmail: string;
		authorName: string;
		authorUrl: string;
		buildTime: string;
...
const renderLink = (href: string, text: string) => h('a', { href, target: '_blank', class: 'vbird-link' }, { default: () => text });
...
} = __VBIRD_ADMIN_METADATA__ || {};

const vbirdDescriptionItems: DescriptionItem[] = [
	{
		content: version,
		title: '版本号'
	},
	...
	{
		content: renderLink(VBIRD_DOC_URL, '点击查看'),
		title: '文档地址'
	},
	{
		content: renderLink(VBIRD_PREVIEW_URL, '点击查看'),
		title: '预览地址'
	},
	{
		content: renderLink(VBIRD_GITHUB_URL, '点击查看'),
		title: 'Github'
	},
	{
	...
	<Page :title="title">
		<template #description>
			<p class="text-foreground mt-3 text-sm leading-6">
				<a :href="VBIRD_GITHUB_URL" class="vbird-link" target="_blank">
					{{ name }}
				</a>
				{{ description }}
			</p>
		...
		<div class="card-box p-5">
			...
			<div class="mt-4">
				<dl class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
					<template v-for="item in vbirdDescriptionItems" :key="item.title">
						<div class="border-border border-t px-4 py-6 sm:col-span-1 sm:px-0">
							...
							<dd class="text-foreground mt-1 text-sm leading-6 sm:mt-2">
								<VbirdRenderContent :content="item.content" />
							</dd>
						</div>
					</template>
				</dl>
