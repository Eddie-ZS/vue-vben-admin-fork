...
import { usePriorityValues } from '@vbird/hooks';
import { EmptyIcon } from '@vbird/icons';
import { $t } from '@vbird/locales';
import { usePreferences } from '@vbird/preferences';
import { cloneDeep, cn, mergeWithArrayOverride } from '@vbird/utils';

import { VbirdHelpTooltip, VbirdLoading } from '@vbird-core/shadcn-ui';
...
async function init() {
	...
	// form 由 vbird-form代替，所以不适配formConfig，这里给出警告
	const formConfig = gridOptions.value?.formConfig;
	// 处理某个页面加载多个Table时，第2个之后的Table初始化报出警告
	// 因为第一次初始化之后会把defaultGridOptions和gridOptions合并后缓存进State
	if (formConfig && formConfig.enabled) {
		console.warn('[Vbird Vxe Table]: The formConfig in the grid is not supported, please use the `formOptions` props');
	}
	props.api?.setState?.({ gridOptions: defaultGridOptions });
	// form 由 vbird-form 代替，所以需要保证query相关事件可以拿到参数
...
watch(
	...
	() => {
		formApi.setState((prev) => {
			const finalFormOptions: VbirdFormProps = mergeWithArrayOverride({}, formOptions.value, prev);
			return {
				...finalFormOptions,
...
<template>
	<div :class="cn('bg-card h-full rounded-md', className)">
		<VxeGrid
			...
			<template v-if="showToolbar" #toolbar-actions="slotProps">
				<slot v-if="showTableTitle" name="table-title">
					<div class="mr-1 pl-1 text-[1rem]">
						{{ tableTitle }}
						<VbirdHelpTooltip v-if="tableTitleHelp" trigger-class="pb-1">
							{{ tableTitleHelp }}
						</VbirdHelpTooltip>
					</div>
				</slot>
			...
			<!-- form表单 -->
			<template #form>
				<div v-if="formOptions" v-show="showSearchForm !== false" :class="cn('relative rounded py-3', isCompactForm ? 'pb-6' : 'pb-4')">
					<slot name="form">
						<Form>
					...
					<div class="bg-background-deep z-100 absolute -left-2 bottom-1 h-2 w-[calc(100%+1rem)] overflow-hidden md:bottom-2 md:h-3"></div>
				</div>
			</template>
			<!-- loading -->
			<template #loading>
				<slot name="loading">
					<VbirdLoading :spinning="true" />
				</slot>
			</template>
			<!-- 统一控状态 -->
			<template #empty>
